...
exports.SetClassFees = (req, res) => {
    const feesClass = req.body.fees_class.replace(/\./g, '');
    const feesAmount = parseInt(req.body.fees_amount);
    const minFeesAmount = 500000;
    let vmsg = '';

    const classRegex = /^[S][1-6]$/i;
    const feesRegex = /^\d+$/;

    if(! classRegex.test(feesClass)){
        vmsg = `Invalid class specified. Please specify one of: S1, S2, S3, S4, S5 or S6.`;
    }

    if(! (feesRegex.test(feesAmount) && (feesAmount>=minFeesAmount))){
        vmsg = vmsg+` Invalid fees amount specified. Please specify a valid amount no less than ${minFeesAmount}`;
    }
    ...
    ClassFees.create({ feesClass: feesClass, feesAmount: feesAmount })
    .then(data => { res.send(data) })
    .catch(err => {
        res.status(500).send({
            status: "Error",
            status_code: 101,
            message: err.message || `Error occured while setting fees for ${feesClass}`
        });
    });
}
...
exports.RegisterPayment = async(req, res) => {
    ...
    Students.findOne({
        ...
        FeesPayments.create(fees_payment)
        .then( data => {
            res.send({
                status: "Success",
                status_code: 100,
